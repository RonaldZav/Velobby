...
**Velobby** is a modern, lightweight, and feature-rich lobby management plugin for **Velocity Proxy**. It provides advanced load balancing, auto-reconnection systems, and full customization support.
...
*   **Multi-Language**: Built-in English (`en`) and Spanish (`es`) support.
...
Velobby uses **MiniMessage** for text formatting. You can use gradients, hex colors, and more in `lang/en.yml` or `lang/es.yml`.
...
3.  It finds a suitable lobby (excluding the server the player was kicked from).
4.  It sends the player to that lobby and displays a title/message explaining the redirection.
